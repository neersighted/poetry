...
class VirtualEnvironment:
    def __init__(self, path: Path) -> None:
        self._path = path
        if WINDOWS:
            # XXX: subprocess cannot be called on WindowsPath on CPython <= 3.7
            self._python = str(self._path.joinpath("Scripts/python.exe"))
        else:
            self._python = self._path.joinpath("bin/python")
    ...
    @classmethod
    def make(cls, path: Path) -> "VirtualEnvironment":
        import venv

        builder = venv.EnvBuilder(clear=True, with_pip=True, symlinks=False)
        builder.ensure_directories(path)
        builder.create(path)

        env = cls(path)

        # We add a special file so that Poetry can detect its own virtualenv
        env.path.joinpath("poetry_env").touch()

        # We do this here to ensure outdated pip does not trigger older bugs
